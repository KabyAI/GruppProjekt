...
START_DATE_S = os.getenv("START_DATE", DEFAULT_START_DATE).strip()
...
def _normalize_bq_type(type_name: str | None) -> str | None:
    if type_name is None:
        return None
    upper = type_name.upper()
    return _TYPE_SYNONYMS.get(upper, upper)
    ...
    try:
        ...
        for field in schema:
            ...
            if current is None:
                ...
                _normalize_bq_type(current.field_type)
                != _normalize_bq_type(field.field_type)
...
def ensure_dataset(client: bigquery.Client, project: str, dataset: str, location: str) -> None:
    ...
    try:
        ...
        log("INFO", "Created dataset", dataset=dataset_id, location=location)
        return

    current_location = (existing.location or "").lower()
    expected_location = (location or "").lower()
    if expected_location and current_location and current_location != expected_location:
        log(
            "CRITICAL",
...
def run_ingestion() -> None:
    ...
    if not BQ_LOCATION:
        log("CRITICAL", "BQ_LOCATION must be set")
        raise SystemExit(1)
    ...
    ensure_dataset(client, PROJECT, BQ_DATASET, BQ_LOCATION)
