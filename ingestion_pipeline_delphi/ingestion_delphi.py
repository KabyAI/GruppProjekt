...
import pandas as pd
import requests
try:  # Optional dependency for unit tests where bigquery not installed
    ...
    if TYPE_CHECKING:  # pragma: no cover
        ...
        class _MissingBigQueryClient:
            def __getattr__(self, name: str) -> Any:  # noqa: D401
                raise ImportError(
                    "google-cloud-bigquery not installed; install dependencies to use ingestion functions."
                )

        class _MissingBigQueryModule:
            Client = _MissingBigQueryClient  # type: ignore
            SchemaField = object  # type: ignore
            TimePartitioning = object  # type: ignore
            TimePartitioningType = type("TimePartitioningType", (), {"DAY": "DAY"})  # type: ignore

        bigquery = _MissingBigQueryModule()  # type: ignore
...
def table_schema() -> List["bigquery.SchemaField"]:  # type: ignore[name-defined]
    return [
...
def ensure_table(client: "bigquery.Client", project: str, dataset: str, table: str) -> str:  # type: ignore[name-defined]
...
def table_row_count(client: "bigquery.Client", table_id: str) -> int:  # type: ignore[name-defined]
...
def merge_upsert(client: "bigquery.Client", staging_id: str, table_id: str) -> None:  # type: ignore[name-defined]
    sql = f"""
    MERGE `{table_id}` T
    USING `{staging_id}` S
...
def existing_epiweeks(
    client: "bigquery.Client",  # type: ignore[name-defined]
    table_id: str,
    region: str,
    start_epiweek: int,
    end_epiweek: int,
...
def upsert_rows(client: "bigquery.Client", df: pd.DataFrame, table_id: str) -> int:  # type: ignore[name-defined]
    if df.empty:
        return 0
...
def main() -> None:
    print("[DEBUG] Delphi main() started", flush=True)
    run_ingestion()
    print("[DEBUG] Delphi run_ingestion() completed", flush=True)


if __name__ == "__main__":
    print("[DEBUG] Delphi script starting", flush=True)
    main()
    print("[DEBUG] Delphi script finished", flush=True)
